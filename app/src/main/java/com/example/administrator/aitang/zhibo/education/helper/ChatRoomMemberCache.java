...
public class ChatRoomMemberCache {
    ...
    private boolean sendReceiveMemPermissions(IMMessage message) {
        if (message.getAttachment() != null
            ...
            attachment.getMeetingOptCommand();
    ...
    private Observer<CustomNotification> customNotification = new Observer<CustomNotification>() {
        ...
        public void onEvent(CustomNotification customNotification) {
            Log.d("TAG", "chatroomMemberCache.java" + "customNotification()");
            ...
            if (command == MeetingOptCommand.SPEAK_ACCEPT.getValue() || command == 0) {//11
                for (MeetingControlObserver observer : meetingControlObservers) {
                    observer.onAccept(roomId);
                }
            } else if (command == MeetingOptCommand.ALL_STATUS.getValue()) {//1
                // 刚入群，主持人发所有成员权限给观众
                for (MeetingControlObserver observer : meetingControlObservers) {
                    observer.onSaveMemberPermission(roomId, accounts);
                }
            } else if (command == MeetingOptCommand.STATUS_RESPONSE.getValue()) {//3
                // 向所有人请求有权限的成员。有权限的成员返回的通知。
                for (MeetingControlObserver observer : meetingControlObservers) {
                    observer.onPermissionResponse(roomId, accounts);
                }
            } else if (command == MeetingOptCommand.SPEAK_REJECT.getValue()) {//12
                // 主持人拒绝/挂断连麦
                for (MeetingControlObserver observer : meetingControlObservers) {
                    observer.onReject(roomId);
                }
            } else if (command == MeetingOptCommand.SPEAK_REQUEST.getValue()) {//10
                // 有人举手发言
                for (MeetingControlObserver observer : meetingControlObservers) {
                    observer.onHandsUp(roomId, customNotification.getFromAccount());
                }
            } else if (command == MeetingOptCommand.SPEAK_REQUEST_CANCEL.getValue()) {//13
                // 取消举手发言
                for (MeetingControlObserver observer : meetingControlObservers) {
