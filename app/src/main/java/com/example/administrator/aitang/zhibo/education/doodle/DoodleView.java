...
import com.example.administrator.aitang.zhibo.education.doodle.action.Action;
import com.example.administrator.aitang.zhibo.education.doodle.action.MyFillCircle;
...
public class DoodleView extends SurfaceView implements SurfaceHolder.Callback, TransactionObserver {
    ...
    public Map<String, List<Transaction>> getUserDataMap() {
        return userDataMap;
    }
    ...
    private DoodleChannel myInitPlaybackChannel() {
        DoodleChannel palybackChannel;
        palybackChannel = new DoodleChannel();
        palybackChannel.paintSize = this.paintSize;
        palybackChannel.setType(this.paintType);
        return palybackChannel;
    }
    ...
    public void onTransaction(String account, final List<Transaction> transactions) {
        ...
        DoodleChannel playbackChannel = initPlaybackChannel(account);//初始化涂鸦通道
    ...
    private void onPaintBackground() {
        ...
        mCanvas.drawColor(bgColor);  // 涂鸦板背景颜色
        if (!isClick())
            surfaceHolder.unlockCanvasAndPost(mCanvas);
    }
    ...
    private void onActionStart(float x, float y) {
        ...
        channel.action.onDraw(mCanvas);
        if (mCanvas != null && !isClick()) {
            surfaceHolder.unlockCanvasAndPost(mCanvas);
        }
    }
    ...
    private void onMultiTransactionsDraw(DoodleChannel playbackChannel, String account, List<Transaction> transactions) {
        ...
        if (!isClick()) {
            nCanvas = surfaceHolder.lockCanvas();
        }
        for (Transaction t : transactions) {
            switch (t.getStep()) {
                case Transaction.ActionStep.LASER_PEN:
                    // 绘制背景
                    nCanvas.drawColor(bgColor);
                    ...
                    laserChannel.action.onDraw(nCanvas);
                    break;
        ...
        for (Transaction t : transactions) {
            switch (t.getStep()) {
                case Transaction.ActionStep.START:
                    ...
                    playbackChannel.action.onStart(nCanvas);
                    playbackChannel.action.onDraw(nCanvas);
                    break;
                case Transaction.ActionStep.MOVE:
                    saveUserData(account, t, false, false, false);
                    if (playbackChannel.action != null) {
        ...
        Log.d("TAG", "isclick:" + isClick() + "showdown:" + isShowDown());
        if (nCanvas != null && !isClick()) {
            surfaceHolder.unlockCanvasAndPost(nCanvas);
            setShowDown(true);
        } else {
            setShowDown(false);
        }
