...
//            RTSTunData channelData = new RTSTunData(sessionId, toAccount, data.getBytes
            RTSTunData channelData = new RTSTunData(sessionId, null, data.getBytes
                    ("UTF-8"), data.getBytes().length);
            boolean isSend = RTSManager2.getInstance().sendData(channelData);
            Log.i("TAG", "TransactionCenter.class--发送的白板数据内容 = " + index + ", BYTES = " + data.getBytes().length + ", isSend=" + isSend);
     ...
     * 数据解析
     */
    public void dataParsing(String sessionId, String account, String data) {
...
//        }
        observers.get(sessionId).onTransaction(account, transactions);
        if (step == Transaction.ActionStep.LASER_PEN || step == Transaction.ActionStep.LASER_PEN_END) {//标记||标记结束
            return;
        }
