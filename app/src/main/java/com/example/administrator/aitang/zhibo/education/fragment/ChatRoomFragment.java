package com.example.administrator.aitang.zhibo.education.fragment;
...
import com.example.administrator.aitang.zhibo.education.adapter.ChatRoomTabPagerAdapter;
...
import com.example.administrator.aitang.zhibo.education.helper.MsgHelper;
import com.example.administrator.aitang.zhibo.education.helper.VideoListener;
import com.example.administrator.aitang.zhibo.education.module.FullScreenType;
import com.example.administrator.aitang.zhibo.education.module.MeetingConstant;
...
import com.example.administrator.aitang.zhibo.im.ui.tab.FadeInOutPageTransformer;
import com.example.administrator.aitang.zhibo.im.ui.tab.PagerSlidingTabStrip;
import com.example.administrator.aitang.zhibo.im.ui.tab.reminder.ReminderId;
import com.example.administrator.aitang.zhibo.im.ui.tab.reminder.ReminderItem;
import com.example.administrator.aitang.zhibo.permission.MPermission;
import com.example.administrator.aitang.zhibo.permission.annotation.OnMPermissionDenied;
import com.example.administrator.aitang.zhibo.permission.annotation.OnMPermissionGranted;
import com.example.administrator.aitang.zhibo.permission.annotation.OnMPermissionNeverAskAgain;
import com.example.administrator.aitang.zhibo.permission.util.MPermissionUtil;
...
//    private ViewGroup[] viewLayouts = new ViewGroup[3];
    ...
    public void onAttach(Context context) {
        ...
        this.activity = (ZhiboActivity) context;
...
//        viewLayouts[2] = thirdRightVideoLayout;
    ...
    public void initLiveVideo(ChatRoomInfo roomInfo, String channelName, final boolean isCreate, String shareUrl, ModuleProxy moduleProxy) {
        ...
        AVChatManager.getInstance().joinRoom2(roomId, AVChatType.VIDEO, new AVChatCallback<AVChatData>() {
            ...
            public void onSuccess(AVChatData avChatData) {
                logd("加入多人房间成功:" + avChatData.getExtra());
            ...
            public void onFailed(int i) {
                if (i == 404)
                    logd("加入多人房间失败,对象不存在，roomid=" + roomId);
                else
                    logd("加入多人房间失败,code=" + i);
            }

            @Override
            public void onException(Throwable throwable) {
                logd("加入多人房间异常,throwable=" + throwable.getMessage());
            }
        });
    ...
    public void onOnlineStatusChanged(boolean isOnline) {
        ...
        NIMClient.getService(ChatRoomService.class).fetchRoomInfo(roomId).setCallback(new RequestCallback<ChatRoomInfo>() {
            ...
            public void onSuccess(ChatRoomInfo chatRoomInfo) {
                ...
                ((ZhiboActivity) getActivity()).setRoomInfo(roomInfo);
                updateDeskShareUI();
            }
    ...
    ChatRoomMemberCache.RoomMsgObserver roomMsgObserver = new ChatRoomMemberCache.RoomMsgObserver() {
        ...
        public void onMsgIncoming(List<ChatRoomMessage> messages) {
            ...
            for (IMMessage message : messages) {
                if (message.getFromAccount().equals(roomInfo.getCreator())) {
                    String creatorContent = message.getContent();
                    if (creatorContent == null)
                        return;
                    if (creatorContent.equals("我开启摄像头了")) {
                        cancelFullScreen(true);
                        return;
                    } else if (creatorContent.equals("我关闭摄像头了")) {
    ...
    Observer<RTSControlEvent> controlObserver = new Observer<RTSControlEvent>() {
        ...
        public void onEvent(RTSControlEvent rtsControlEvent) {
            // your code
            Toast.makeText(activity, "onevent", Toast.LENGTH_SHORT).show();
        }
    };
    ChatRoomMemberCache.MeetingControlObserver meetingControlObserver = new ChatRoomMemberCache.MeetingControlObserver() {
        ...
        public void onAccept(String roomID) {
            ...
            chooseSpeechType();
            RTSManager2.getInstance().observeControlNotification(roomID, controlObserver, true);
        }
    ...
    ChatRoomMemberCache.RoomMemberChangedObserver roomMemberChangedObserver = new ChatRoomMemberCache.RoomMemberChangedObserver() {
        ...
        public void onRoomMemberIn(ChatRoomMember member) {
            onMasterJoin(member.getAccount(), true);
    ...
    private void chooseSpeechType() {
        ...
        AVChatManager.getInstance().muteLocalVideo(true);//关闭视频

        ChatRoomMemberCache.getInstance().setRTSOpen(false);//关闭白板互动
    ...
    @Override
    public void onUserJoined(final String s) {
        userJoinedList.add(s);
        long startTime = System.currentTimeMillis();
// 从现在开始，查询20条消息
        NIMClient.getService(ChatRoomService.class).pullMessageHistory(roomId, startTime, 10).setCallback(new RequestCallback<List<ChatRoomMessage>>() {
            @Override
            public void onSuccess(List<ChatRoomMessage> chatRoomMessages) {
                ...
                for (ChatRoomMessage msg : chatRoomMessages) {
                    time = msg.getTime();
                    Log.d("TAG", "历史消息：" + msg.getContent() + msg.getTime());
                    String con = msg.getContent();
                    if (con != null) {
                        if (con.equals("我关闭摄像头了")) {
                            onMasterJoin(s, false);
                            return;
                        } else if (con.equals("我开启摄像头了")) {
                            onMasterJoin(s, true);
                            return;
                        }
                    }
                }
                NIMClient.getService(ChatRoomService.class).pullMessageHistory(roomId, time, 20).setCallback(this);
            }
    ...
    public void onCallEstablished() {
        ...
        onMasterJoin(DemoCache.getAccount(), true);
    }
     ...
     *
     * @param s
     * @param flag 是否添加主持人到画布
     */
    private void onMasterJoin(String s, boolean flag) {
        if (userJoinedList != null && userJoinedList.contains(s) && s.equals(roomInfo.getCreator())) {
            if (masterRender == null) {
                ...
                if (isSetup && masterRender != null) {
                    addIntoMasterPreviewLayout(masterRender, flag);
                    ChatRoomMemberCache.getInstance().savePermissionMemberbyId(roomId, roomInfo.getCreator());
                    updateDeskShareUI();
                }
            }
    ...
    // 将主持人添加到主持人画布
    private void addIntoMasterPreviewLayout(SurfaceView surfaceView, boolean flag) {
...
//        removeViews();
    ...
    private void cancelFullScreen(boolean flag) {
        if (!flag) {
            fullScreenImage.setVisibility(View.VISIBLE);
        ...
        addIntoMasterPreviewLayout(masterRender, true);
        showView();
    }
